...
class CrawlerClient {
  ...
  getDeadletter(urn) {
    return this._call('get', `${this.url}/deadletters/${encodeURIComponent(urn)}`, `Failed to get deadletter`);
  }

  deleteDeadletter(urn) {
    return this._call('delete', `${this.url}/deadletters/${encodeURIComponent(urn)}`, `Failed to delete deadletter`);
  }

  requeueDeadletter(urn, queue) {
    return this._call('delete', `${this.url}/deadletters/${encodeURIComponent(urn)}?requeue=${encodeURIComponent(queue)}`, `Failed to get deadletter`);
  }
